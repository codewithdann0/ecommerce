...
@section('content')
    ...
    <form method="GET" action="{{ route('products.index') }}" class="mb-4">
        ...
        <button type="submit" style="background-color: blue" class="text-white p-2 rounded">Search</button>
    </form>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach ($products as $product)
            <div class="bg-white p-4 rounded shadow">
                @if ($product->image)
                    <img src="{{ asset($product->image) }}" alt="{{ $product->name }}" class="w-full h-auto mb-4 rounded">
                @else
                    <img src="https://via.placeholder.com/150" alt="Placeholder image" class="w-full h-auto mb-4 rounded">
                @endif
